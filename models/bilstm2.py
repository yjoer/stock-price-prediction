...
def train_model_bilstm2(X_train, y_train, X_val, y_val):
    es = EarlyStopping(monitor="val_loss", patience=25, restore_best_weights=True)
    model = build_model_bilstm2()
...
def cv_model_bilstm2(X_train, y_train):
    histories = []
    losses = dict()
    ...
    for train_index, val_index in tscv.split(X_train):
        ...
        history = train_model_bilstm2(X_train_cv, y_train_cv, X_val_cv, y_val_cv)
        histories.append(history)

    for i, history in enumerate(histories):
        losses[f"fold_{i}"] = history.history

    with open("logs/cross_val_bilstm2.json", "w") as f:
