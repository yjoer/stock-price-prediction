...
def train_model_lstm2(X_train, y_train, X_val, y_val):
...
def cv_model_lstm2(X_train, y_train):
    histories = []
    losses = dict()
    ...
    for train_index, val_index in tscv.split(X_train):
        ...
        history = train_model_lstm2(X_train_cv, y_train_cv, X_val_cv, y_val_cv)
        histories.append(history)

    for i, history in enumerate(histories):
        losses[f"fold_{i}"] = history.history

    with open("logs/cross_val_lstm2.json", "w") as f:
